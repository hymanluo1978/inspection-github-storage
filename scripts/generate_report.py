...
def create_inspection_sheet(wb, records, sheet_name, key):
    ws = wb.create_sheet(sheet_name)
    headers = ['记录ID', '项目名称', '楼号', '房号', '验收人', '验收结论', '描述', '照片']
    ws.append(headers)
    for record in records:
        if 'inspections' in record and key in record['inspections']:
...
def create_tile_sheet(wb, records):
    ...
    headers = ['记录ID', '项目名称', '楼号', '房号', '实测人', '实测时间', '空鼓', '高低差', '垂直度', '平整度', '阴阳角']
    ws.append(headers)
    for record in records:
...
def create_putty_sheet(wb, records):
    ...
    headers = ['记录ID', '项目名称', '楼号', '房号', '实测人', '实测时间', '顶棚极差', '阴阳角', '垂直度', '平整度', '三边两线', '阴阳角直线度']
    ws.append(headers)
    for record in records:
